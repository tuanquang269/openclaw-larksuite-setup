...
2. **Tương tác với User:** Thông báo cho User biết rằng trên Terminal vừa xuất hiện một mã QR. Yêu cầu User mở app LarkSuite/Feishu quét mã đó.
3. **CẤP QUYỀN BẮT BUỘC (CRITICAL):** Dù QR code đã tạo Bot tự động, bảo mật của Lark thường chặn quyền nhận tin nhắn. Bạn PHẢI dặn User:
   - Truy cập **Lark Developer Console** -> Chọn App vừa tạo.
   - Vào mục **Events and Callbacks**, kiểm tra xem có event `im.message.receive_v1` (Receive messages) chưa. Nếu chưa CÓ, BẮT BUỘC phải ấn Add Event và thêm nó vào.
   - Vào mục **Permissions**, đảm bảo cấp quyền `contact:contact.base:readonly` để Bot đọc được tên người chat.
1. Mở kết nối Feishu/Lark bằng lệnh:
   `openclaw channels login --channel feishu`
