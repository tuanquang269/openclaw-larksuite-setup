...
4. **Thiết lập Xác thực (Authentication):** Hệ thống này hỗ trợ cả API Key và OAuth (được bọc hoàn toàn qua CLI của OpenClaw):
   - **Nếu dùng API Key:** Inject key vào hệ thống (thông qua lệnh `openclaw auth` hoặc can thiệp trực tiếp vào file credentials).
   - **Nếu dùng OAuth (ví dụ: Claude web, OpenAI Codex):** Hãy tự động chạy lệnh `openclaw auth login --provider <tên-provider>` để mở luồng đăng nhập OAuth cho User.
