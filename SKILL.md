...
4. **Thiết lập Xác thực (Authentication):**
   - **ƯU TIÊN 1 (Dùng API Key):** Rất an toàn, không bị ban. Cấu hình trực tiếp bằng `openclaw auth` hoặc sửa file credentials.
   - **CẢNH BÁO NGUY HIỂM KHI DÙNG OAUTH:** Việc dùng lệnh CLI (như `openclaw auth login`) để ép đăng nhập OAuth vào các web models (như Claude, OpenAI) rất dễ làm User **bị khóa/ban account** do vi phạm chính sách chống tự động hóa (headless browser).
   - **Cách lách luật OAuth an toàn (CrawBot pattern):** Nếu User bắt buộc dùng OAuth, tuyệt đối KHÔNG dùng CLI thuần. Hãy dùng cách "bọc CLI" thành 1 ứng dụng Desktop App (như CrawBot): Tải file `.dmg` -> Kéo vào Applications -> Chuột phải chọn Open để lách Gatekeeper. Các App có giao diện WebKit gốc này sẽ giúp đăng nhập OAuth mà không bị hệ thống AI phát hiện là bot.
