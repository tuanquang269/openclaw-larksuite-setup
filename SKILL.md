...
4. **Thiết lập Xác thực (Authentication):**
   - **ƯU TIÊN 1 (Dùng Subscription OAuth - Vì chi phí rẻ hơn):** 
     - **CẢNH BÁO:** Tuyệt đối KHÔNG dùng lệnh CLI thuần (như `openclaw auth login`) để đăng nhập OAuth, vì sử dụng headless browser rất dễ làm User bị ban/khóa account (vi phạm chính sách).
     - **Cách thực hiện an toàn (CrawBot pattern):** Hướng dẫn User tải ứng dụng bọc CLI dạng Desktop App (như CrawBot `.dmg`). Kéo file vào Applications -> Chuột phải chọn Open để lách Gatekeeper. Việc đăng nhập qua giao diện WebKit gốc của App này sẽ bảo vệ tài khoản an toàn 100% không bị hệ thống AI nhận diện là bot.
   - **ƯU TIÊN 2 (Dùng API Key - Phương án dự phòng):** Nếu User không có gói subscription, hãy cấu hình API Key thông qua `openclaw auth` hoặc sửa file credentials. Phương án này an toàn nhưng đắt đỏ hơn.
...
4. **KIỂM TRA BẢO MẬT (RẤT QUAN TRỌNG):** Dù QR code thao tác tự động được 90%, cấu hình bảo mật mặc định của Lark có thể thay đổi theo thời gian và chặn Bot nhận tin nhắn. BẠN PHẢI DẶN USER làm thêm thao tác sau để chắc chắn 100% có thể chat được ngay lập tức:
   - Truy cập trang **Lark Developer Console** -> Chọn App vừa tạo.
   - Vào mục **Events and Callbacks**, BẮT BUỘC phải chắc chắn đã subscribe event **"Receive messages"** (Nhận tin nhắn, ví dụ: `im.message.receive_v1`, `v2`, hoặc các version mới hơn). Nếu chưa có thì phải tự Add Event này vào.
   - Vào mục **Permissions**, BẮT BUỘC cấp quyền liên quan đến **"Read user info / Contact data"** (ví dụ: `contact:contact.base:readonly` hoặc tương đương tùy theo version mới nhất của Lark) để Bot có thể đọc được tên người đang chat.
...
- Nếu User báo Bot không trả lời trên Lark: Kiểm tra `/tmp/openclaw/openclaw-*.log`. Nếu thấy Log thông báo WebSocket Connected nhưng KHÔNG có bất kỳ event nào liên quan đến nhận tin nhắn, nghĩa là App chưa subscribe event. Hãy hướng dẫn User vào Lark Developer Console -> Chọn App -> Events and Callbacks -> Add Event **"Receive messages"** (tùy theo Lark mà nó có thể tên là `im.message.receive_v1`, `v2`, v.v...).
